...
// TestPushRepoMemoryJobHasNoConcurrencyGroup verifies that the push_repo_memory job is not
// serialized through a job-level concurrency group. GitHub Actions keeps at most one pending job
// per concurrency group and cancels the rest, so a shared group silently dropped the memory of
// every run beyond the second whenever a fan-out of workers finished concurrently. Concurrent
// pushes converge through the push script's compare-and-swap + merge + retry loop instead.
func TestPushRepoMemoryJobHasNoConcurrencyGroup(t *testing.T) {
	...
	assert.Empty(t, pushJob.Concurrency,
		"push_repo_memory must not declare a concurrency group: GitHub cancels all but one pending job per group, losing memory pushes under fan-out")
}
